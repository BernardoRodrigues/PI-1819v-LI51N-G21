module.exports = function() {
    ...
    function getArtists(artist, cb, page = 1) {
        if (!artist || !artist.name || artist.name.length === 0)
            throw new InvalidParametersError('Artist name can not be null')
        return request(
    ...
    function handleResponse(err, mapper, body, cb) {
        if (err) {
            if (err.error === 500) throw new ServerDownError('Last fm Api is down')
        }
    ...
    function getAlbumInfo(album, cb) {
        if (!album || !album.mbid || album.mbid.length === 0)
            throw new InvalidParametersError('Album mbid can not be null')
        return request(
    ...
    function getTopAlbums(artist, cb, page = 1) {
        if (!artist || !artist.mbid || artist.mbid.length === 0)
            throw new InvalidParametersError('Artist mbid can not be null')
        return request(
    ...
    function mapArtists(result) {
        return result.results.artistmatches.artist.map(a => 
    ...
    function mapTopAlbums(result) {
        ...
        return result.topalbums.album.map(a => 
            Album.init(a.name, a.url, a.mbid, a.image.map(i => Image.init(i['#text'])), 
                Artist.init(a.artist.name, a.artist.mbid, a.artist.url, []))
        )
    }
