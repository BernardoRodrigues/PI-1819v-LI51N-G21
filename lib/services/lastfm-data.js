...
module.exports = function(request) {
    ...
    async function getArtists(artist, page = 1) {
        if (!artist || !artist.name || artist.name.length === 0)
            throw new InvalidParametersError('Artist name can not be null', 400)
        const result = await request
                        .get(endpoints.getArtistsEndpoint(artist.name, page), {json: true})
                        .catch(handleServerError)
        return await mapArtists(result)
    }
    ...
    async function getAlbumInfo(album) {
        ...
        const result = await request
            .get(endpoints.getAlbumInfoEndpoint(album.mbid), {json: true})
            .catch(handleServerError)
        return await mapAlbumInfo(result)
    }

    async function getTopAlbums(artist, page = 1) {
        if (!artist || !artist.mbid || artist.mbid.length === 0)
            throw new InvalidParametersError('Artist mbid can not be null', 400)
        const result = await request
                            .get(endpoints.getTopAlbumsEndpoint(artist.mbid, page), {json: true})
                            .catch(handleServerError)
        return await mapTopAlbums(result)
    }
