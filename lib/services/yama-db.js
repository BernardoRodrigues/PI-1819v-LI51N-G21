module.exports = function() {
    ...
    function deletePlaylist(playlist, cb) {
        ...
        request.delete(endpoints.deleteTracksListEndpoint(playlist.id), (err, resp, firstBody) => {
            if (err && resp.statusCode === 404) 
                throw new InvalidParametersError(`Playlist with id ${playlist.id} was not found`, 400)
            request.delete(
                endpoints.deletePlaylistEndpoint(playlist.id),
                (err, resp, body) => {
                    if (err) throw new InvalidParametersError(`Playlist with id ${playlist.id} was not found`, 400)
    ...
    function getPlaylists(cb) {
        return request.get(
            ...
            (err, resp, body) => 
            handleResponse(err, mapMultiplePlaylists(JSON.parse(body)), cb)
        )
    }
