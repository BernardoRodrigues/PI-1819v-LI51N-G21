module.exports = function (request) {
    ...
    async function updatePlaylist(playlist) {
        if (!playlist || !playlist.id || playlist.id.length === 0)
            throw new InvalidParametersError('Playlist id is required', 400)
        // const result = await request.get(endpoints.getPlaylistEndpoint(playlist.id), {
        //     json: true,
        // }).then(checkResult).then(mapPlaylist).catch(handleError)
        const body = {
            name: playlist.name,
            description: playlist.description
        }
        return request
            ...
            .then(() => {
                playlist.name = playlist.name || body.name
                playlist.description = playlist.description || body.description
                return playlist
            })
            .then(mapPlaylist)
            .catch(handleError)
